...
<img src="/architecture/gcp-data-platform-es-light.svg" alt="Diagrama de la plataforma de datos en GCP en tres bandas horizontales. Arriba, donde se escribe cada regla: GitHub Actions con fmt, validate, compile y frescura; Terraform con sus 12 módulos, que crea los controles y se va; y el Knowledge Catalog con la taxonomía, los policy tags y el glosario, creado por Terraform. En el medio, quién dice que no y qué devuelve cuando lo dice: «Bronze es inmutable» por if_generation_match=0, que responde 412 Precondition Failed; «la doc está al día» por generar_diccionario.py --check, que hace que el PR no pase; «Gold reconcilia con Silver» por las assertions de grano y de total, tras las cuales Dataform no construye lo que dependía del modelo que falló; «la columna es confidencial» por el policy tag de la taxonomía, que devuelve Access Denied on policy tag y no lo hereda ni el owner del proyecto; y «el consumo entra por Gold», porque la service account de BI no tiene grant sobre Silver. Abajo, el camino del dato: las fuentes —una API SaaS paginada y una planilla de Google Sheets— entran a la ingesta de Cloud Functions Gen2, una por fuente y ninguna transforma; de ahí a Bronze en Cloud Storage, crudo y append-only con linaje; después Dataform como motor único, con SQLX versionado y el grafo derivado de los ref(); luego Silver y Gold en BigQuery; y por último la capa de acceso con vistas autorizadas y RLS, dibujada en punteado porque está diseñada y no implementada, hasta el tablero de BI. Las flechas punteadas bajan desde la banda de arriba hasta los controles —Terraform y el catálogo instalan, Dataform declara y aplica— y las flechas llenas bajan desde cada control hasta el tramo del camino que puede frenar." class="dark:hidden max-w-none m-0" width="2334" height="365" />
<img src="/architecture/gcp-data-platform-es-dark.svg"  alt="Diagrama de la plataforma de datos en GCP en tres bandas horizontales. Arriba, donde se escribe cada regla: GitHub Actions con fmt, validate, compile y frescura; Terraform con sus 12 módulos, que crea los controles y se va; y el Knowledge Catalog con la taxonomía, los policy tags y el glosario, creado por Terraform. En el medio, quién dice que no y qué devuelve cuando lo dice: «Bronze es inmutable» por if_generation_match=0, que responde 412 Precondition Failed; «la doc está al día» por generar_diccionario.py --check, que hace que el PR no pase; «Gold reconcilia con Silver» por las assertions de grano y de total, tras las cuales Dataform no construye lo que dependía del modelo que falló; «la columna es confidencial» por el policy tag de la taxonomía, que devuelve Access Denied on policy tag y no lo hereda ni el owner del proyecto; y «el consumo entra por Gold», porque la service account de BI no tiene grant sobre Silver. Abajo, el camino del dato: las fuentes —una API SaaS paginada y una planilla de Google Sheets— entran a la ingesta de Cloud Functions Gen2, una por fuente y ninguna transforma; de ahí a Bronze en Cloud Storage, crudo y append-only con linaje; después Dataform como motor único, con SQLX versionado y el grafo derivado de los ref(); luego Silver y Gold en BigQuery; y por último la capa de acceso con vistas autorizadas y RLS, dibujada en punteado porque está diseñada y no implementada, hasta el tablero de BI. Las flechas punteadas bajan desde la banda de arriba hasta los controles —Terraform y el catálogo instalan, Dataform declara y aplica— y las flechas llenas bajan desde cada control hasta el tramo del camino que puede frenar." class="hidden dark:block max-w-none m-0" width="2334" height="365" />
</div>
<figcaption class="text-xs font-mono text-magi-muted mt-2">Las flechas punteadas que bajan son la distancia entre declarar una regla y aplicarla. Modelado con LikeC4 y exportado con Graphviz; la fuente está en <code>architecture/gcp-data-platform/</code>.</figcaption>
</figure>
